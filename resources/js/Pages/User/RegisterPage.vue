<template>
	<main>
		...
		<div class="min-h-screen flex flex-col">
			<div class="container max-w-sm mx-auto flex-1 flex flex-col items-center ">
					...
					<div class="px-6" v-for="(formStep, step) in steps" :key="step">
						<div v-if="currentStep === step">
							<div v-for="(field, i) in formStep" :key="i">
								<FormInputSelect v-if="field.type === 'select'" class="lg:w-full " :label="field.label"
									v-model="form[field.key]" :error="form.errors[field.key]">
									<option value="">Select {{ field.label }}</option>
									<option v-for="(option, index) in field.options" :key="index" :value="option.value">{{ option.label }}
									</option>
								</FormInputSelect>

								<FormInputCnic v-else-if="field.type === 'cnic'" :label="field.label" class="lg:w-full"
									:error="form.errors[field.key]" v-model="form[field.key]" />
								<FormInputText v-else :label="field.label" class="lg:w-full" :error="form.errors[field.key]"
									v-model="form[field.key]" :type="field.type" />
							</div>
						</div>
					</div>
					<div class="flex gap-2 justify-between mt-5 p-6 items-center">
						<div>
							<button v-if="isLastStep" class="btn-main" :disabled="form.processing" type="submit" form="create-form">
								Create Account
							</button>
						</div>
						<div>
							...
							<button v-if="!isLastStep" class="btn btn-primary" @click.prevent="nextStep">
								Next
							</button>
						</div>
					</div>
